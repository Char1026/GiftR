...
function onDeviceReady() {
    if (!localStorage.getItem("char1026")) {
        localStorage.setItem("char1026", JSON.stringify(people));
    } else {
        people = JSON.parse(localStorage.getItem("char1026"));
        console.log(people);
    }
...
function displayContacts(){
    ...
    people.forEach(function (value) {
        ...
        li.addEventListener("touchstart", function(ev){
                person = a.getAttribute("person-id");
                console.log(person);
                editContact(person);
        })
    })                    
